...
namespace
{
    ...
    bool isJoystick(udev_device* udevDevice)
    {
        ...
        // Our last resort if udev really can't provide us with definitive information
        if (std::strstr(devnode, "/js"))
            return true;

        // At this point, assume it's not a joystick
        return false;
    }
        ...
        if (udevDevice)
        {
            ...
            if (action)
            {
                if (isJoystick(udevDevice))
                {
                    ...
                    if ((record == joystickList.end()) && std::strstr(action, "add"))
                    {
